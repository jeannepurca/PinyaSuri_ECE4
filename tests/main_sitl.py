...
from mock_camera import MockImageCapture as ImageCapture
from mock_classifier import TFLiteClassifier
from flight_metrics import FlightMetricsLogger

# Configuration Paths  
BASE_DIR = Path(__file__).resolve().parent
LOG_DIR = BASE_DIR / "logs_sitl"
IMG_DIR = BASE_DIR / "images_sitl"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "flight_sitl.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("PinyaSuri-SITL")


class PinyaSuriSystem:
    """Main system coordinator for PinyaSuri - SITL Version"""
    ...
    async def initialize(self, max_retries: int = 3) -> bool:
        ...
        SITL version - connects to simulator
        """
        
        logger.info("="*60)
        logger.info("SITL SIMULATION MODE")
        logger.info("="*60)
        
        config.ensure_directories()
        
        # Initialize Pixhawk (SITL)
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Connecting to SITL (attempt {attempt}/{max_retries})...")
                logger.info(f"SITL Address: {config.PIXHAWK_ADDRESS}")
                ...
                logger.info("✓ SITL connected successfully")
                break
            except Exception as e:
                logger.error(f"✗ SITL connection failed: {e}")
                if attempt == max_retries:
                    logger.error("Maximum retry attempts reached for SITL.")
                    logger.error("\nTroubleshooting:")
                    logger.error("1. Make sure Mission Planner SITL is running")
                    logger.error("2. Check connection string: " + config.PIXHAWK_ADDRESS)
                    logger.error("3. Try: udp://:14540 or tcp://127.0.0.1:5760")
                    return False
                await asyncio.sleep(2)

        # Initialize Mock Camera
        try:
            logger.info("Initializing Mock Camera (SITL)...")
        ...
        # Initialize AI Classifier (Real or Mock)
        try:
            ...
            logger.error(f"✗ AI Classifier loading failed: {e}")
            return False
        ...
        logger.info("="*60)
        logger.info("ALL SYSTEMS READY - SITL MODE")
        logger.info("="*60)
    ...
    async def run_mission(self):
        ...
        SITL version - works with simulated flight
        """
        ...
        logger.info("📡 Upload a mission in Mission Planner and ARM the vehicle")
    ...
    async def _process_waypoint(self, waypoint_idx: int, mission_total: int, position: Optional[dict]):
        ...
        try:
            ...
            # Run AI Classification (real or mock)
...
async def main():
    """Main entry point - SITL version"""
    
    system = PinyaSuriSystem()
    
    # Initialize system
    logger.info("=== PinyaSuri SITL System Starting ===")
    
    if not await system.initialize():
        logger.error("System initialization failed. Exiting.")
        logger.error("\nSITL Setup Instructions:")
        logger.error("1. Start Mission Planner SITL")
        logger.error("2. Make sure SITL is listening on UDP port 14540")
        logger.error("3. Try running: python3 main_sitl.py")
        return 1
    ...
    logger.info("=== PinyaSuri SITL System Stopped ===")
    return 0
